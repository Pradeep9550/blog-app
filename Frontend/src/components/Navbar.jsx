...
const Navbar = () => {
  ...
  const handleSearchChanger = async (e) => {
    ...
    const res = await axios.get(`https://blog-app-backend-ianr.onrender.com/api/auth/search?q=${value}`)
    const data = res.data
  ...
  return (
    <nav className="bg-white fixed z-50 top-0 left-0 right-0 border-b border-gray-200 dark:bg-gray-900">
      <div className="max-w-screen-xl flex items-center justify-between mx-auto p-6">
        ...
        {/* Desktop Home + Search Bar */}
        <div className="hidden lg:flex items-center space-x-4">
          {login && (
            <>
              <Link to="/" className="text-sm text-gray-800 dark:text-white p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
                Home
              </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative group">
            <button className="flex text-sm bg-gray-800 rounded-full focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-600">
              <img
                ...
                src={login ? userStore.user.profilePic : 'https://static.vecteezy.com/system/resources/thumbnails/005/129/844/small/profile-user-icon-isolated-on-white-background-eps10-free-vector.jpg'}
                alt="user"
              />
            </button>

            <div className="absolute right-0 z-50 hidden group-hover:block bg-white border rounded shadow-lg dark:bg-gray-800 dark:border-gray-700">
              {login && (
          ...
          {/* Three-dot Mobile Menu (Visible Only if Logged In) */}
          {login && (
            <div className="relative md:hidden group">
              ...
              <div className="absolute right-0 z-50 hidden group-hover:flex flex-col w-56 bg-white p-4 border border-gray-200 rounded shadow-md dark:bg-gray-800 dark:border-gray-600">
                <Link
                  to="/"
